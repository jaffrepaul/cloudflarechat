...
app.post('/api/project/install', async (req: Request, res: Response) => {
  try {
    ...
    const result = await projectManager.installDependencies(projectId, packageManager);
...
// Get project status
app.get('/api/project/:projectId/status', async (req: Request, res: Response) => {
  try {
...
// Configure Sentry
app.post('/api/project/configure-sentry', async (req: Request, res: Response) => {
  try {
    const { projectId, sentryDsn } = req.body;
    
    if (!projectId || !sentryDsn) {
      return res.status(400).json({ error: 'Missing required fields: projectId, sentryDsn' });
    }
