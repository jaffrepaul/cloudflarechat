    ...
    try {
      ...
      if (error instanceof DOMException && error.name === 'QuotaExceededError') {
        ...
        try {
          ...
          console.error('Failed to save to sessionStorage even after clearing logs');
        }
      }
    }
  },
...
export function useProjectManager() {
  ...
  useEffect(() => {
    if (activeProject) {
      safeSessionStorage.setItem('activeProject', JSON.stringify(activeProject));
    }
  }, [activeProject]);
  ...
  const fetchProjects = useCallback(async () => {
    try {
      ...
      if (response.ok) {
        ...
        if (activeProject) {
          const updated = projectsData.find((p: Project) => p.id === activeProject.id);
          if (updated) {
  ...
  // Get project status
  const getProjectStatus = useCallback(async (projectId: string): Promise<Project | null> => {
    try {
      const response = await fetch(`${BUILD_SERVER_URL}/api/project/${projectId}/status`);
      if (response.ok) {
